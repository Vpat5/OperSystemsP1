...
void test_list_is_circular(void)
{
  populate_list();
  node_t *last = lst_->head->prev;
  TEST_ASSERT_TRUE(last->next == lst_->head);
  TEST_ASSERT_TRUE(lst_->head->prev == last);
}

void test_list_is_doubly_linked(void)
{
  populate_list();
  node_t *curr = lst_->head->next;
  node_t *prev = lst_->head;
  while (curr != lst_->head) {
    TEST_ASSERT_TRUE(curr->prev == prev);
    prev = curr;
    curr = curr->next;
  }
}
...
int main(void) {
  ...
  RUN_TEST(test_list_is_doubly_linked);
  return UNITY_END();
}
